...
module.exports = options => {
	...
	const prompts = [
		...
		},
		{
			type: 'list',
			name: 'identifier',
			message: 'Which pre-release identifier should be used?',
			when: answers => (version.isPrereleaseIncrement(answers.version) && answers.version !== 'prerelease') || (answers.version === 'prerelease' && !version.isAlreadyPrereleaseVersion(oldVersion)),
			choices: answers => version.PRERELEASE_IDENTIFIERS
				.map(identifier => ({
					name: `${identifier} ${chalk.gray.dim(`(${version.getNewVersion(oldVersion, answers.version, identifier)})`)}`,
					value: identifier
				}))
		},
		{
		...
		},
		{
			...
			message: answers => {
				...
				const newVersion = version.getNewVersion(oldVersion, answers.version, answers.identifier);

				return `Will bump from ${chalk.cyan(oldVersion)} to ${chalk.cyan(newVersion + tagPart)}. Continue?`;
			}
		}
	];
	...
	return inquirer
		...
		.then(answers => Object.assign({}, options, answers, {
			version: version.getNewVersion(oldVersion, answers.version, answers.identifier)
		}));
};
